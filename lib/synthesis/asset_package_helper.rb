module Synthesis
  module AssetPackageHelper
    ...
    def javascript_include_merged(*sources)
      ...
      merged_tags = sources.collect {|source| javascript_include_tag(source, options) }.join("\n")
      if merged_tags.respond_to?(:html_safe)
        merged_tags.html_safe
      else
        merged_tags
      end
    end
    ...
    def stylesheet_link_merged(*sources)
      ...
      merged_tags = sources.collect { |source| stylesheet_link_tag(source, options) }.join("\n")
      if merged_tags.respond_to?(:html_safe)
        merged_tags.html_safe
      else
        merged_tags
